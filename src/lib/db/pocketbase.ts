...
export async function getServers(userId: string) {
  const memberList = await pb
    .collection<MemberWithServer>("members")
    .getFullList({
      filter: `user = "${userId}"`,
      expand: "server",
    });
...
export async function createSubjectServer(
  ...
  const students = await pb
    .collection<StudentModel>(Collections.Students)
    .getFullList({
  ...
  const lecturerId = (
    await pb
      .collection<LecturerModel>(Collections.Lecturers)
      .getFirstListItem(`user = "${ownerId}"`)
  ).id;
...
export async function createClubServer(
  servername: string,
  ownerId: string,
  image: any
) {
...
export async function createGeneralServer(
  servername: string,
  ownerId: string,
  branch: string,
  sem: string,
...
export async function getCourse(serverId: string) {
  const course = await pb
    .collection<CourseModel>(Collections.Courses)
...
export async function fetchStudents(serverId: string) {
  return await pb.send<AttendenceStudent[]>("/custom/serverstudents", {
    serverId: serverId,
  });
}
...
export async function fetchAttendence(serverId: string, date: string) {
  const course = await pb
    .collection<CourseModel>(Collections.Courses)
    .getFirstListItem(`server = "${serverId}"`);
  const attendenceList = await pb
    .collection<AttendenceWithStudent>(Collections.Attendence)
    .getFullList({
...
export function getDateString(date: Date) {
  return `${date.getUTCFullYear()}-${(date.getUTCMonth() + 1)
    ...
    .getUTCDate()
    .toString()
    .padStart(2, "0")} 00:00:00.000Z`;
}

export async function getEnrolledCourses(studentUserId: string) {
  return await pb.send<CourseModel[]>("/custom/unique/enrolledcourses", {
